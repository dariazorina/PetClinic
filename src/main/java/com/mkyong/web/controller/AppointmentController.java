...
import com.mkyong.web.model.dto.AppointmentDtoInput;
...
public class AppointmentController {
    ...
    public ModelAndView edit(@RequestParam(value = "id", required = false) Integer id,
                             @RequestParam(value = "pet_id", required = false) Integer pet_id, HttpServletRequest request) {
...
//    public ModelAndView edit(@RequestParam(value = "client_id", required = false) Integer client_id, @RequestParam(value="pet_id", required=false) Integer pet_id) {
    public ModelAndView edit(AppointmentDtoInput inputAppointment, HttpServletRequest request) {

        Pet pet = petService.getById(inputAppointment.getPet().getId());

        Map<String, String> validationResult = UtilsApi.fieldsValidation(inputAppointment);
        if (validationResult.size() == 0) {

            Appointment appointment = new Appointment();
            appointment.setPet(inputAppointment.getPet());
            appointment.setId(inputAppointment.getId());
            appointment.setDoctor(inputAppointment.getDoctor());

            SimpleDateFormat formatter = new SimpleDateFormat("MM/dd/yyyy", Locale.ENGLISH);
            String dateInString = inputAppointment.getDate();
            ...
            if (inputAppointment.getId() == null) { //this means we want to save a new appointment
            ...
            modelAndView.addObject("appointment", inputAppointment);
...
//        return "{\"status\":\"ok\"}";
//        } else {//            return "{\"status\":\"Error\", \"message\":\"Invalid Input Data on Server\"}";     }
    }
